...
class SIRLMStudioAgent {
  constructor(options = {}) {
    ...
    console.log("🔮 H3X Super Intelligent Regulator (SIR) Agent initialized with LMStudio integration");
  ...
  async analyzeMessage(message) {
    ...
    if (lowerMessage.includes('analyze') || lowerMessage.includes('sir analysis') || 
        lowerMessage.includes('environmental scan') || lowerMessage.includes('ai readiness')) {
      ...
      if (lowerMessage.includes('ai readiness') || lowerMessage.includes('readiness')) analysisType = 'ai_readiness';
    ...
    if (lowerMessage.includes('simulate') || lowerMessage.includes('simulation') || 
        lowerMessage.includes('environment') || lowerMessage.includes('test scenario')) {
      analysis.requiresTools = true;
    ...
    // Monitoring patterns
    if (lowerMessage.includes('monitor') || lowerMessage.includes('status') || 
        lowerMessage.includes('health') || lowerMessage.includes('check')) {
      analysis.requiresTools = true;
    ...
    if (lowerMessage.includes('supervision') || lowerMessage.includes('approval') || 
        lowerMessage.includes('human intervention') || lowerMessage.includes('review')) {
      analysis.requiresTools = true;
...
Respond in a professional yet engaging manner, providing clear insights and actionable recommendations.`;
  ...
  buildUserPrompt(userMessage, toolResults, context) {
    ...
    prompt += "Please provide a comprehensive response based on the user's request and any tool results above.";
  ...
  generateFallbackResponse(userMessage, toolResults) {
    ...
    if (toolResults.length > 0) {
      ...
      response += "I'm ready to assist with SIR analysis, environmental simulations, monitoring, and supervision tasks. Please specify what you'd like me to analyze or simulate.";
    }
    ...
    switch (toolName) {
      case 'SIRAnalysis':
        if (result.environmentalData) {
          this.systemState.environmentData = { ...this.systemState.environmentData, ...result.environmentalData };
        }
  ...
  extractEnvironment(message) {
    const envPatterns = ['laboratory', 'office', 'industrial', 'outdoor', 'indoor', 'testing', 'production'];
  ...
  extractPriority(message) {
    ...
    if (message.toLowerCase().includes('low') || message.toLowerCase().includes('minor')) return 'low';
    return 'medium';
  }
}

module.exports = { SIRLMStudioAgent };
