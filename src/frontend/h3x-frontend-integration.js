...
class H3XFrontendIntegration {
  ...
  async loadInitialData() {
    try {
      ...
      // Load configuration
      const config = await this.fetchAPI('/config', 'GET', null, this.options.apiBaseUrl.replace('/api', ''));
  ...
  updateAmendmentsUI(amendments) {
    const logElement = document.getElementById('stateLog') || document.getElementById('amendments-log');
    ...
    // Add amendments
    amendments.slice(-20).reverse().forEach(amendment => {
  ...
  async exportArchive(loopType, filename = null) {
    try {
      ...
      const result = await this.fetchAPI('/archives/export', 'POST', { loopType, filename });
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = H3XFrontendIntegration;
