  ...
  evaluateEnvironmentCompliance(environmentData) {
    ...
    Object.keys(this.environmentStandards.physical).forEach(metric => {
      ...
      if (current !== undefined) {
        ...
        if (isOptimal) {
          compliantMetrics++;
          compliance.details[metric] = { status: "OPTIMAL", current, standard: standard.optimal };
        } else if (isAcceptable) {
          compliantMetrics += 0.7; // Partial compliance
          compliance.details[metric] = { status: "ACCEPTABLE", current, standard: standard.optimal };
          compliance.recommendations.push(`Optimize ${metric} to reach optimal range ${standard.optimal[0]}-${standard.optimal[1]} ${standard.unit}`);
        } else {
          compliance.details[metric] = { status: "OUT_OF_RANGE", current, standard: standard.range };
          compliance.recommendations.push(`CRITICAL: ${metric} is outside acceptable range (${standard.range[0]}-${standard.range[1]} ${standard.unit})`);
...
module.exports = { HexperimentFramework };
