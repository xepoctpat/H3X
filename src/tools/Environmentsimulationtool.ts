  ...
  func: async ({ simulationType, parameters, standards }) => {
    ...
    switch (simulationType) {
      ...
      case 'validate':
        ...
        simulationResult.status = validationSuccess
      ...
      default:
        ...
        simulationResult.data = {
          message:
            'Simulation type not recognized. Available types: standard, status, validate, complete',
        };
    }
    ...
    Object.keys(standards).forEach((key) => {
      if (parameters[key] !== undefined && standards[key]) {
        ...
        if (Array.isArray(standard.optimal)) {
          optimal =
            parameters[key] >= standard.optimal[0] && parameters[key] <= standard.optimal[1];
 ...
 */
async function protocolDrivenSimulation(protocolScenario): Promise<any> {
