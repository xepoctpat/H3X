...
const monitoringTool = {
  ...
  description: "Monitors environmental conditions and system status according to real-life standards",
  schema: z.object({
...
async function startMonitoring(systemState, timestamp) {
  ...
  const monitoringScenario = {
    ...
    description: "Continuous monitoring of environmental parameters to ensure compliance with RL standards",
    startTime: timestamp,
    status: "ACTIVE",
    parameters: [
      {
...
async function getMonitoringStatus(systemState, timestamp) {
...
async function stopMonitoring(systemState, timestamp) {
  const finalReport = {
