...
import { dataIntegration } from '../utils/data-integration';
...
export class VisualizationManager {
  ...
  private startPerformanceMonitoring(): void {
    const monitor = () => {
      ...
      if (this.performanceStats.lastFrameTime > 0) {
        ...
        // Running average
        this.performanceStats.averageFPS = this.performanceStats.averageFPS * 0.9 + fps * 0.1;
  ...
  private adjustQuality(direction: 'up' | 'down'): void {
    ...
    console.log(
      `H3X Visualization: Adjusting quality ${direction} (FPS: ${this.performanceStats.averageFPS.toFixed(1)})`,
    );
  }
  ...
  public getStats(): {
    ...
    const baseStats = this.renderer?.getStats() || {
      nodes: 0,
      triads: 0,
      fps: 0,
    };
