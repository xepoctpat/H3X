 ...
 * Comprehensive health monitoring for the Hexperiment Labs system
 */
...
class HealthChecker {
    ...
    async checkFileSystem() {
        ...
        if (missing.length === 0) {
            return { status: 'pass', message: 'All critical files and directories present' };
        } else {
    ...
    async checkNodeModules() {
        ...
        if (missingDeps.length === 0) {
            return { status: 'pass', message: `All ${Object.keys(dependencies).length} dependencies installed` };
        } else {
            return { status: 'warn', message: `Missing dependencies: ${missingDeps.slice(0, 3).join(', ')}${missingDeps.length > 3 ? '...' : ''}` };
        }
    }
    ...
    async checkPorts() {
        ...
        const portChecks = await Promise.all(
            requiredPorts.map(port => this.isPortAvailable(port))
        );

        const busyPorts = requiredPorts.filter((port, index) => !portChecks[index]);
        
        if (busyPorts.length === 0) {
            return { status: 'pass', message: `All required ports (${requiredPorts.join(', ')}) available` };
        } else {
            return { status: 'warn', message: `Ports in use: ${busyPorts.join(', ')}` };
        }
    }
    ...
    async checkGit() {
        try {
            ...
            if (changedFiles === 0) {
                ...
                return { status: 'warn', message: `${changedFiles} files have changes` };
            }
        } catch (error) {
    ...
    async checkEnvironment() {
        ...
        if (existingEnvFiles.length > 0) {
            return { status: 'pass', message: `Environment files found: ${existingEnvFiles.join(', ')}` };
        } else {
    ...
    async checkPublicFiles() {
        ...
        if (htmlFiles.length > 0) {
            return { status: 'pass', message: `${htmlFiles.length} HTML files in Public directory` };
        } else {
    ...
    async checkScripts() {
        ...
        return { status: 'pass', message: `${jsFiles.length} script files available` };
    }
    ...
    async run() {
        ...
        this.log(`📊 Overall Health Score: ${totalScore.toFixed(1)}%`, totalScore > 80 ? 'pass' : totalScore > 60 ? 'warn' : 'fail');
