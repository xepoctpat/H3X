document.addEventListener('DOMContentLoaded', function() {
  ...
  ws.onmessage = (msg) => {
    ...
    div.className = 'task';
    div.innerHTML = `<span class="time">[${data.time ? new Date(data.time).toLocaleTimeString() : new Date().toLocaleTimeString()}]</span>` +
  ...
  sendBtn.onclick = async () => {
    ...
    if (Math.random() > 0.6) {
      ...
      hopeDiv.innerHTML = `<span class="type">HOPE</span> <span class="payload">Vizuelizujem uspeh zadatka </span>`;
    ...
    humanDiv.innerHTML = `<span class="type">HUMAN</span> <span class="payload">🫱 Svaki zadatak je važan. Tvoj doprinos je primećen. 🫲</span>`;
