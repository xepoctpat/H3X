...
volumes:
  ...
  updateHealthMetrics() {
    ...
    document.querySelector('[data-metric="lmstudio"]').style.width =
      `${this.systemHealth.lmstudio}%`;
  ...
  async cloneDatabase() {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/db/clone', {
        method: 'POST',
      });
      if (response.ok) {
...
function scanContainers() {
  controlCenter.scanContainers();
}
function createNetwork() {
  controlCenter.log('Creating AI network...', 'info');
}
function orchestrateContainers() {
  controlCenter.log('Orchestrating container stack...', 'info');
}
function generateCompose() {
  controlCenter.generateCompose();
}
function launchStack() {
  controlCenter.log('Launching Docker stack...', 'info');
}
function resetStack() {
  controlCenter.log('Resetting Docker stack...', 'warning');
}
function connectContainer(name) {
  controlCenter.connectContainer(name);
}
function configureContainer(name) {
  controlCenter.log(`Configuring container: ${name}`, 'info');
}
function cloneDatabase() {
  controlCenter.cloneDatabase();
}

// LMStudio Functions
function testLMStudioConnection() {
  controlCenter.testLMStudioConnection();
}
function loadModels() {
  controlCenter.loadModels();
}
function sendTestPrompt() {
  controlCenter.sendTestPrompt();
}
function clearTest() {
...
function performSoftReset() {
  controlCenter.performSoftReset();
}
function performMediumReset() {
  controlCenter.performMediumReset();
}
function performHardReset() {
  controlCenter.performHardReset();
}
function performFactoryReset() {
  controlCenter.performFactoryReset();
}
function confirmReset() {
  controlCenter.confirmReset();
}
function cancelReset() {
  controlCenter.cancelReset();
}

// System Functions
function clearLogs() {
  controlCenter.clearLogs();
}
function exportLogs() {
  controlCenter.exportLogs();
}
function refreshLogs() {
  controlCenter.refreshLogs();
}
...
function showBanner(message, type = 'info', timeout = 3000) {
  ...
  banner.style.background =
    type === 'error' ? '#ffdddd' : type === 'success' ? '#ddffdd' : '#ddeeff';
...
async function generatePersonaFrontend(): Promise<any> {
    ...
    try {
      ...
      if (healthResponse.ok) {
        ...
        this.addProtocolMessage(
          'system',
          `🔬 Protocol Server connected: ${healthData.service} v${healthData.version}`,
        );
        ...
        try {
          ...
          switch (data.type) {
            case 'persona_response':
              this.addProtocolMessage(
                'persona',
                `Persona(s) generated: ${JSON.stringify(data.data.personas, null, 2)}`,
              );
            ...
            case 'lmstudio_response':
              this.addProtocolMessage(
                'lmstudio',
                `LMStudio: ${JSON.stringify(data.data.response)}`,
              );
              break;
...
async function connectProtocolServer(): Promise<any> {
...
async function runDockerContainer(): Promise<any> {
