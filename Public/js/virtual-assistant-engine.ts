...
Respond concisely but helpfully, focusing on practical suggestions. Use a professional yet friendly tone. Reference neural and cognitive science principles when relevant.`;
  ...
  analyzeWorkPatterns() {
    ...
    const isWorkingHours =
      currentHour >= this.userPreferences.workingHours.start &&
  ...
  getProductivityInsights(tasks) {
    ...
    if (tasks.length > 10) {
      insights.push(
        "📊 Task overload detected. Consider using the 'Rule of 3' - focus on only 3 major tasks per day.",
      );
    }
    ...
    if (urgentTasks.length > 5) {
      insights.push(
        '⚠️ High urgent task ratio. Review priorities to prevent burnout and maintain quality.',
      );
    }
