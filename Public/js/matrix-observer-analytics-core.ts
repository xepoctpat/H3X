document.addEventListener('DOMContentLoaded', function () {
  ...
  ws.onmessage = (msg) => {
    ...
    div.className = 'event';
    div.innerHTML =
      `<span class="time">[${data.time ? new Date(data.time).toLocaleTimeString() : ''}]</span>` +
  ...
  ws.onopen = () => {
    ...
    div.className = 'event';
    div.innerHTML =
      '<span class="type">INFO</span> <span class="payload">Spectator connected. Čekam događaje...</span>';
    logDiv.appendChild(div);
  };
  ...
  ws.onclose = () => {
    ...
    div.className = 'event';
    div.innerHTML =
      '<span class="type">INFO</span> <span class="payload">Spectator disconnected.</span>';
    logDiv.appendChild(div);
  };
});
