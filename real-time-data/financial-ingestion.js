...
class FinancialIngestionService {
    constructor() {
        ...
        this.updateInterval = parseInt(process.env.UPDATE_INTERVAL) || 60; // 1 minute
        this.isRunning = true;
    ...
    async collectStockData() {
        for (const symbol of this.symbols.filter(s => !s.includes('-USD'))) {
            try {
                ...
                console.log(`📈 Collected ${symbol}: $${stockData.data.price} (${stockData.data.changePercent}%)`);
    ...
    async collectCryptoData() {
        for (const symbol of this.symbols.filter(s => s.includes('-USD'))) {
            try {
                ...
                console.log(`₿ Collected ${symbol}: $${cryptoData.data.price} (${cryptoData.data.change24h?.toFixed(2)}%)`);
        ...
        for (const index of this.indices) {
            try {
                ...
                const indexData = {
                    ...
                    metrics: {
                        marketSentiment: this.calculateMarketSentiment(parseFloat(quote['10. change percent'].replace('%', '')))
                    }
                };

                await this.storeFinancialData(indexData);
                console.log(`📊 Collected ${index}: $${indexData.data.price} (${indexData.data.changePercent}%)`);
    ...
    async storeFinancialData(financialData) {
        ...
        await this.redis.lpush('feedback:financial:queue', JSON.stringify({
            type: 'financial',
            data: financialData,
            timestamp: Date.now()
        }));
