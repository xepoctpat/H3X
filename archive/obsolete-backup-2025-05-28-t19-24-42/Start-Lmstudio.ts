...
import { H3XCodeGenerator } from './src/generators/codeGenerator';
import { HSPImaginationEngine } from './src/protocol/hspImaginationEngine';
...
const app = express();
const PORT = (process.env as ProcessEnv).PORT || 3979;
...
const sirAgent = new SIRLMStudioAgent({
  baseURL: (process.env as ProcessEnv).LMSTUDIO_URL || 'http://localhost:1234/v1',
  model: (process.env as ProcessEnv).LMSTUDIO_MODEL || 'local-model',
});
  ...
  try {
    ...
    if (instructions.type === 'interactive') {
      ...
      return res.json({
        ...
        message:
          'Interaktivna imaginacija prepoznata. Prikaži korisniku UI elemente i čekaj akciju.',
      });
    }
    ...
    if (instructions.type === 'proactive') {
      const event = {
        type: 'proactive',
        imagination,
        suggestions: instructions.proactiveSuggestions,
        time: new Date().toISOString(),
      };
      broadcastSpectator(event);
      return res.json({
    ...
    switch ((generationType || instructions.type || '').toLowerCase()) {
      case 'ai_assistant':
        console.log(
          '🟪 [GENERATION] Generating AI Assistant with:',
      ...
      case 'simulation':
        console.log(
          '🟪 [GENERATION] Generating Simulation with:',
          JSON.stringify(parameters || {}),
        );
      ...
      case 'analysis_tool':
        console.log(
          '🟪 [GENERATION] Generating SIR Analysis Tool with:',
          JSON.stringify(parameters || {}),
        );
      ...
      case 'endpoint':
        ...
        result = await codeGenerator.generateServerEndpoint(
          parameters?.endpointName || 'CustomEndpoint',
          parameters?.functionality || 'Custom functionality',
        );
        break;
      case 'project':
        console.log('🟪 [GENERATION] Generating Project with:', JSON.stringify(parameters || {}));
        result = await codeGenerator.generateProjectStructure(
          parameters?.projectName || 'H3X Generated Project',
          parameters?.projectType || 'ai_assistant',
        );
        break;
      case 'data_simulator':
        console.log(
          '🟪 [GENERATION] Generating Data Simulator with:',
          JSON.stringify(parameters || {}),
        );
        result = await codeGenerator.generateRealtimeDataSimulator(
          parameters?.dataType || 'environmental',
...
wss.on('connection', (ws) => {
  ...
  ws.send(
    JSON.stringify({
      type: 'info',
      message: 'Connected to H3X Spectator. Svi događaji će biti prikazani uživo.',
    }),
  );
  ws.on('close', () => {
