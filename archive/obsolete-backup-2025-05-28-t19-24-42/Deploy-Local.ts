...
console.log(
  '🔬 Deploying Hexperiment Labs SIR Control Interface (No OpenAI Version)...\n'
);
...
(process.env as ProcessEnv).USE_OPENAI = 'false';
...
try {
  ...
  packageJson.scripts = {
    ...packageJson.scripts,
    'no-openai':
      'node -r dotenv/config ./src/index.js dotenv_config_path=./env/.env.standalone',
    'no-openai:dev':
      'nodemon --inspect=9239 --signal SIGINT -r dotenv/config ./src/index.js dotenv_config_path=./env/.env.standalone',
