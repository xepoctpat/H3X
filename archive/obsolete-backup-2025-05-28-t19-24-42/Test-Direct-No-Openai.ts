...
import { sirAgent } from './src/agent-no-openai';
...
class DirectSIRTestClient {
  ...
  async testAgentDirectly(mockContext) {
    ...
    import { processSIRRequest } from './src/agent-no-openai';
...
if (import.meta.url === `file://${process.argv[1]}`) {
  async function runDirectTests(): Promise<any> {
