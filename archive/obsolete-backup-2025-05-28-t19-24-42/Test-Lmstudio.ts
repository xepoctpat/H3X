...
async function testConnection(): Promise<any> {
  try {
    ...
    console.log(
      `${colors.yellow}💡 Make sure to start the server first: npm run lmstudio${colors.reset}`,
    );
    return false;
  }
}
...
async function sendMessage(message, context = {}): Promise<any> {
  try {
...
// Direct tool testing
async function testTool(endpoint, payload): Promise<any> {
  try {
...
// Show system status
async function showStatus(): Promise<any> {
  try {
  ...
  const askQuestion = () => {
    rl.question(`${colors.magenta}🎯 SIR> ${colors.reset}`, async (input) => {
      ...
      if (command === 'exit' || command === 'quit') {
        console.log(
          `${colors.cyan}👋 Goodbye! SIR Control Interface session ended.${colors.reset}`,
        );
        rl.close();
        return;
      }
