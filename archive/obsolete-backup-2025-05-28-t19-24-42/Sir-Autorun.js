...
class SIRAutorun {
    ...
    async performanceAnalysis() {
        try {
            const response = await this.agent.processMessage('Analyze current system performance metrics');
            return {
                status: 'ANALYZED',
    ...
    printStats() {
        ...
        console.log(`  Success rate: ${this.stats.tasksCompleted > 0 ? 
            ((this.stats.tasksSucceeded / this.stats.tasksCompleted) * 100).toFixed(1) : 0}%`);
