...
class H3XLMStudioNPMIntegration {
    ...
    log(message, type = 'info') {
        ...
        console.log(`${colors[type]}[H3X-NPM] ${message}${colors.reset}`);
    }

    // Run npm script and get LM Studio response
    async runNpmWithLMStudio(scriptName, prompt, options = {}) {
        try {
    ...
    async dockerLMStudioIntegration() {
        ...
        for (const workflow of workflows) {
            const result = await this.runNpmWithLMStudio(
                workflow.script, 
                workflow.prompt,
                { dockerMode: true }
            );
    ...
    startBackgroundService(scriptName) {
    ...
    async executeDockerCompose(command) {
        return new Promise((resolve, reject) => {
            this.log(`Executing: docker-compose ${command}`, 'info');
            
            const process = spawn('docker-compose', command.split(' '), {
                cwd: this.projectRoot
            });
...
module.exports = { H3XLMStudioNPMIntegration };
